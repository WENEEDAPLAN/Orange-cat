...
namespace Рыжик
{
    public partial class Level3 : Form
    {
        ...
        public Level3()
        {
            ...
            MaximizeBox = false;
            Level3Timer.Tick += new EventHandler(MoveEnemies);
            Level3Timer.Tick += new EventHandler(MovePlatforms);
            Level3Timer.Tick += new EventHandler(Gravity);
            Level3Timer.Tick += new EventHandler(UpdateAnimation);
            Level3Timer.Tick += new EventHandler(ObjectInCollusion);
            KeyDown += new KeyEventHandler(KeyIsPress);
            KeyUp += new KeyEventHandler(KeyIsUp);
            Level3Timer.Interval = 30;
            PlayMusic();
            Init();
        }
        ...
        {
            music.SoundLocation = Path.Combine(new DirectoryInfo(Directory.GetCurrentDirectory()).Parent.Parent.FullName.ToString(), "Music\\stray_10-Communication.wav");
            music.PlayLooping();
        }
