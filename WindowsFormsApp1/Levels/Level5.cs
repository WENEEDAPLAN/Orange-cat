...
namespace Рыжик
{
    public partial class Level5 : Form
    {
        ...
        public Level5()
        {
            ...
            MaximizeBox = false;
            Level5Timer.Tick += new EventHandler(MovePlatforms);
            Level5Timer.Tick += new EventHandler(MoveEnemies);
            Level5Timer.Tick += new EventHandler(Gravity);
            Level5Timer.Tick += new EventHandler(UpdateAnimation);
            Level5Timer.Tick += new EventHandler(ObjectInCollusion);
            KeyDown += new KeyEventHandler(KeyIsPress);
            KeyUp += new KeyEventHandler(KeyIsUp);
            Level5Timer.Interval = 30;
            PlayMusic();
            Init();
        }
        ...
        {
            pictureBox5.Left += Vel_x;
            if (pictureBox5.Left < 0 || pictureBox5.Left + pictureBox5.Width > this.ClientSize.Width)
            {
                Vel_x *= -1;
            }

            pictureBox6.Top += Vel_y;
            if (pictureBox6.Top < 0 || pictureBox6.Top > 500)
            {
                Vel_y = -Vel_y;
            }
        }
        ...
        {
            music.SoundLocation = Path.Combine(new DirectoryInfo(Directory.GetCurrentDirectory()).Parent.Parent.FullName.ToString(), "Music\\stray_27-Clean-City.wav");
            music.PlayLooping();
        }
